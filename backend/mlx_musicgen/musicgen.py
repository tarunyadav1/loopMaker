...
# Vendored from mlx-examples/musicgen
# Modified: imports use local .encodec and .t5 modules
...
class MusicGen(nn.Module):
    ...
    def generate(
        ...
        Args:
            ...
            progress_callback: Optional callable(step, max_steps) called
                periodically during generation.
        ...
        # Report progress every ~2% of steps (at least every 10 steps)
        report_interval = max(1, max_steps // 50)
        for offset in tqdm(range(max_steps)):
            ...
            if progress_callback and offset % report_interval == 0:
                progress_callback(offset + 1, max_steps)
